...
class _DetailsScreenState extends State<DetailsScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => MovieDetailsBloc(getIt<MovieDetailsUseCase>())..add(GetMovieDetailsEvent(widget.movie!.id! , Constants.API_KEY)),
      child: Scaffold(
               ...
               switch (state.movieDetailsState) {
                 case RequestState.isLoading:
                   return const SizedBox(height: 200,
                       child: Center(
                         child: CircularProgressIndicator(color: Colours
                             .SecondryColor,),));
                           ...
                           Row(
                             children: [
                               Icon(Icons.confirmation_num_sharp,
                                 color: Colours.GrayColor, size: 15,),
                               SizedBox(width: 4,),
                               ...
                               SizedBox(width: 20,),
                               Icon(Icons.star, color: Colours.GrayColor,
                                 size: 15,),
                               SizedBox(width: 4,),
                               Text('${state.movieDetails?.voteAverage
                                   ?.toStringAsFixed(2)} (IMDP)',
                                   style: TextStyle(fontFamily: 'AABB',
                                       fontSize: 12,
                                       color: Colours.GrayColor)),
                               ...
                               Column(
                                 ...
                                 children: [
                                   Text('Genre', style: TextStyle(
                                       ...
                                       color: Colors.white)),
                                   SizedBox(height: 8,),
                                   Text('${state.movieDetails?.genres?.first.name}',
                                       style: TextStyle(fontFamily: 'AABB',
                                           fontSize: 12,
                                           color: Colours.GrayColor)),
                                 ],
